...
def fill_slide(prs, slide_index, start_date, end_date, use_line_items=False, by_keyword=False, by_category=False):
    ...
    # DEBUG — print a sample of each so you can see the real field names
    print("=== dataPivot sample ===")
    print(dataPivot[:2])
    print("pivot row keys:", list(dataPivot[0].keys()) if dataPivot else "no pivot data")
    
    print("=== dataCapout sample ===")
    print(dataCapout[:2])
    print("capout row keys:", list(dataCapout[0].keys()) if dataCapout else "no capout data")
    ...
    if use_line_items:
        # 1) Sort your pivot data by spend
        dataPivot.sort(key=lambda x: x["spend"], reverse=True)
    
        # 2) Build a capout_map by line_item_name, but only SUM aggregation
        capout_map = {
            r["line_item_name"]: r
            for r in dataCapout
            if r.get("aggregation") == "sum"
        }
    
        max_items = len(line_item_filter)
        for idx, piv in enumerate(dataPivot[:max_items], start=1):
            # --- fill your pivot metrics as before ---
            replacements[f"{{{{impressions_lineitem_{idx}}}}}"] = f"{piv.get('impressions', 0):,}"
            replacements[f"{{{{clicks_lineitem_{idx}}}}}"]      = f"{piv.get('clicks', 0):,}"
            replacements[f"{{{{spend_lineitem_{idx}}}}}"]       = f"${int(piv.get('spend', 0)):,}"
            replacements[f"{{{{sales_lineitem_{idx}}}}}"]       = f"${int(piv.get('sales', 0)):,}"
            replacements[f"{{{{units_lineitem_{idx}}}}}"]       = f"{piv.get('units', 0):,}"
            replacements[f"{{{{ROAS_lineitem_{idx}}}}}"]        = f"${piv.get('ROAS Demand', 0):,.2f}"
            replacements[f"{{{{CPC_lineitem_{idx}}}}}"]         = f"${piv.get('CPC Demand', 0):,.2f}"
            replacements[f"{{{{name_lineitem_{idx}}}}}"]        = piv.get("line_item_name", f"LineItem {idx}")
    
            # --- lookup the matching capout row by name ---
            name = piv.get("line_item_name")
            cap_row = capout_map.get(name, {})
            if cap_row:
                # alert_hour_current is a float (e.g. 21.8), so cast to int
                ah = int(cap_row.get("alert_hour_current", 0))
                replacements[f"{{{{cap_lineitem_{idx}}}}}"] = format_hour_12h(ah) + " EST"
            else:
                # no match → blank cell
                replacements[f"{{{{cap_lineitem_{idx}}}}}"] = ""
...
def generate_pptx(
    ...
    line_items_pivot: list[int],
    line_items_capout: str,
    slide1_start: str,
    slide1_end: str,
    slide2_start: str,
    slide2_end: str,
    ...
    """
    Produces a deck with:
      • Slide 0 (campaign YTD) using slide1_{start,end}
      • Slide 1 (campaign month) using slide2_{start,end}
      • Slide 2 (line-item month) using slide2_{start,end}
      • Slide 3 (keyword month) using slide2_{start,end}
      • Slide 4 (category month) using slide2_{start,end}

    Returns the path to the saved PPTX.
    """
    # override module-globals
    global retailerID, brandName, campaignID, lineItemsPivot, lineItemsCapout, presentationName
    retailerID      = retailer_id
    brandName       = brand_name
    campaignID      = campaign_id
    lineItemsPivot  = line_items_pivot
    lineItemsCapout = line_items_capout
    presentationName = f"{brand_name} Snapshot Deck (generated).pptx"

    # build the deck
    prs = Presentation("template.pptx")

    # Slide 0: YTD campaign (uses slide1 range)
    fill_slide(prs, 0, slide1_start, slide1_end, use_line_items=False, by_keyword=False, by_category=False)
    
    # Slide 0: YTD campaign (uses slide1 range)
    fill_slide(prs, 2, slide1_start, slide1_end, use_line_items=False, by_keyword=False, by_category=False)
    # Slide 1: monthly campaign (uses slide2 range)
    fill_slide(prs, 3, slide2_start, slide2_end, use_line_items=False, by_keyword=False, by_category=False)
    # Slide 2: monthly line-item
    fill_slide(prs, 4, slide2_start, slide2_end, use_line_items=True,  by_keyword=False, by_category=False)
    # Slide 3: monthly keyword
    fill_slide(prs, 5, slide2_start, slide2_end, use_line_items=False, by_keyword=True,  by_category=False)
    # Slide 4: monthly category
    fill_slide(prs, 6, slide2_start, slide2_end, use_line_items=False, by_keyword=False, by_category=True)
...
# Only run this when you do `python main4.py` directly (won’t fire on import)
if __name__ == "__main__":
    # quick smoke-test with your old, hard-coded values:
    out = generate_pptx(
        retailer_id="906",
        brand_name="Nutpods",
        campaign_id=[214650],
        line_items_pivot=[685422],
        line_items_capout="685422",
        slide1_start="2025-01-01",
        slide1_end="2025-05-06",
        slide2_start="2025-04-01",
        slide2_end="2025-04-30",
    )
    print(f"Created {out}")
