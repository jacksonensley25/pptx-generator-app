...
retailerID = "906"
campaignID = [155556]
lineItemsPivot = [454947]
lineItemsCapout = "454947"
brandName = "SLATE"
...
def fill_slide(prs, slide_index, start_date, end_date, use_line_items=False, by_keyword=False, by_category=False):
    ...
    def merge_and_clean_paragraph(p):
        # 1) get raw text
        ...
        if cleaned != original:
            ...
            fmt = {
                ...
                "bold":   f.bold,
                "italic": f.italic,
                "color":  (f.color.rgb if f.color.type is not None else None),
            }
    
            # 3) remove all existing runs
            for run in list(p.runs):
                p._p.remove(run._r)
    
            # 4) add a single new run with cleaned text
            nr = p.add_run()
            nr.text = cleaned
    ...
    # do the merge+clean on every paragraph, both in text-shapes and tables
    for shape in slide.shapes:
        if shape.has_text_frame:
            for para in shape.text_frame.paragraphs:
                merge_and_clean_paragraph(para)
        if shape.has_table:
...
# Slide 0 & 1: campaign-level
fill_slide(prs, 0, "2025-01-01", "2025-05-1", use_line_items=False, by_keyword=False, by_category=False)
fill_slide(prs, 1, "2025-04-01", "2025-04-30", use_line_items=False, by_keyword=False, by_category=False)

# Slide 2: line-item-level
fill_slide(prs, 2, "2025-04-01", "2025-04-30", use_line_items=True, by_keyword=False, by_category=False)

#Slide 3: keyword level
fill_slide(prs, 3, "2025-04-01", "2025-04-30", use_line_items=False, by_keyword=True, by_category=False)

#Slide 4: category level
fill_slide(prs, 4, "2025-04-01", "2025-04-30", use_line_items=False, by_keyword=False, by_category=True)
